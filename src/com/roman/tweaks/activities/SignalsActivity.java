...
public class SignalsActivity extends PreferenceActivity implements
	...
	private static final String PREF_SIGNAL_COLOR_5 = "signal_color_5";
	private static final String PREF_SIGNAL_COLOR_6 = "signal_color_6";
	...
	Preference mSignalColor5;
	Preference mSignalColor6;
	...
	public void onCreate(Bundle ofLove) {
		...
		mSignalAutoColor = (CheckBoxPreference) prefs
				.findPreference(PREF_AUTO_COLOR);
		mSignalTextStyle = (ListPreference) prefs
				.findPreference(PREF_SIGNAL_TEXT_STYLE);
		...
		mSignalColor5 = prefs.findPreference(PREF_SIGNAL_COLOR_5);
		mSignalColor6 = prefs.findPreference(PREF_SIGNAL_COLOR_6);
		mSignalColorStatic = prefs.findPreference(PREF_SIGNAL_COLOR_STATIC);
		// mShow4GIcon = (CheckBoxPreference)
		// prefs.findPreference(PREF_TOGGLE_4G_ICON);
		// mShow2GIcon = (CheckBoxPreference)
		// prefs.findPreference(PREF_TOGGLE_2G_ICON);
		mShowSignalBars = (CheckBoxPreference) prefs
				.findPreference(PREF_SHOW_SIGNAL_BARS);

		// check enabled settings
		mShowSignalBars.setChecked((Settings.System.getInt(
				getContentResolver(), "tweaks_signal_icon_enabled", 1) == 1));
		mSignalAutoColor.setChecked((Settings.System
		...
		mSignalTextStyle.setValueIndex(Settings.System.getInt(
				getContentResolver(), "tweaks_signal_text_style", 0));
	...
	public ColorPickerDialog generateDialog(String preference) {
		ColorPickerDialog cp = new ColorPickerDialog(this,
				mColorChangeListener, Settings.System.getInt(
						getContentResolver(), preference, Settings.System
								.getInt(getContentResolver(), preference,
										Color.WHITE)));
		return cp;
	}
	...
	public boolean onPreferenceTreeClick(PreferenceScreen screen,
		...
		if (preference == mSignalColor0) {
		...
		} else if (preference == mSignalColor5) {

			signalTextColorPickerFlag = "tweaks_signal_text_color_5_bar";
			generateDialog(signalTextColorPickerFlag).show();
			return true;

		} else if (preference == mSignalColor6) {

			signalTextColorPickerFlag = "tweaks_signal_text_color_6_bar";
			generateDialog(signalTextColorPickerFlag).show();
			return true;
			...
			Settings.System.putInt(getContentResolver(), "tweaks_show_4g_icon",
					(enable ? 1 : 0));
			return true;
			...
			Settings.System.putInt(getContentResolver(), "tweaks_show_2g_icon",
					(enable ? 1 : 0));
			return true; 
	...
	ColorPickerDialog.OnColorChangedListener mColorChangeListener = new ColorPickerDialog.OnColorChangedListener() {
		...
		public void colorUpdate(int color) {
			Settings.System.putInt(getContentResolver(),
					signalTextColorPickerFlag, color);
		}
	...
	public void refreshOptions() {
		if (((CheckBoxPreference) findPreference("signal_automatically_color_pref"))
			...
			findPreference("signal_color_5").setEnabled(true);
			findPreference("signal_color_6").setEnabled(true);
		} else {
			...
			findPreference("signal_color_5").setEnabled(false);
			findPreference("signal_color_6").setEnabled(false);
		}
	}
